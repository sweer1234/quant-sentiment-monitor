...
3. 预处理层进行清洗、去重、时间对齐、标的映射  
4. NLP 层输出情绪分数、事件类型、实体列表  
5. 事件影响引擎完成“事件 -> 资产池 -> 多空方向/置信度”判定  
6. 因子层聚合形成分钟/小时级情绪与影响因子  
7. 信号层进行多因子融合并给出交易建议  
8. 回测层验证策略有效性并沉淀评估报告  
9. 监控层持续跟踪延迟、错误率、信号稳定性
...
- [ ] 采集站点管理（增删改查、启停、分组、批量导入导出）
- [ ] 站点权重管理（source/credibility/timeliness/coverage/noise）
- [ ] 权重变更审计日志与版本回滚
- [ ] 内置默认站点库（国内外重点网站 60+）
- [ ] 文本清洗与标准化
- [ ] 实体识别（国家/央行/行业/公司/商品）
...
- [ ] 准备并校验 `configs/sources_registry.default.yaml`（内置站点）
- [ ] 准备 `configs/sources_registry.override.yaml`（环境自定义）
