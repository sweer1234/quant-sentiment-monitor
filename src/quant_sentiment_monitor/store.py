...
class QuantStore:
    ...
    def _load_state(self) -> bool:
        ...
        try:
            ...
            self.notification_outbox = list(payload.get("notification_outbox", []))
            self.source_poll_state = dict(payload.get("source_poll_state", {}))
    ...
    def _queue_alert_notifications(self, *, alert: dict[str, Any], event: Event) -> None:
        channels = list(alert.get("channels", []))
        p_level = str(alert.get("importance_level", "P2"))
        for channel in channels:
            if channel == "webhook":
                continue
            if p_level == "P2" and channel == "email":
                # P2 默认不发邮件，避免噪音。
                continue
    ...
    def delete_source(self, source_id: str, *, actor: str = "system") -> bool:
        if source_id not in self._source_by_id:
            return False
        before = deepcopy(self._source_by_id[source_id])
        del self._source_by_id[source_id]
        self.sources = [item for item in self.sources if item.get("source_id") != source_id]
        self._record_source_version(source_id=source_id, before=before, after=None, actor=actor, reason="delete_source")
        self._audit("source.delete", actor, {"source_id": source_id})
        self._persist_state()
        return True

    def import_sources(self, rows: list[dict[str, Any]], *, actor: str = "system", merge: bool = True) -> dict[str, Any]:
        imported = 0
        skipped = 0
        for row in rows:
            ...
            if not source_id:
                skipped += 1
                continue
            if not merge and source_id in self._source_by_id:
                skipped += 1
                continue
            self.patch_source(source_id=source_id, patch_data=row, actor=actor, reason="import_sources")
            imported += 1
        self._audit("source.import", actor, {"imported": imported, "skipped": skipped, "merge": merge})
        self._persist_state()
        return {"status": "ok", "imported": imported, "skipped": skipped, "merge": merge}
    ...
    def list_polling_sources(self, *, limit: int = 20) -> list[dict[str, Any]]:
        rows = []
        for source in self.sources:
            if not bool(source.get("enabled", True)):
                continue
            rows.append(deepcopy(source))
        rows.sort(key=lambda x: (int(x.get("tier", 2)), int(x.get("poll_interval_sec", 120))))
        return rows[:limit]

    def mark_source_poll_result(self, source_id: str, *, ok: bool, fetched: int, error: str | None = None) -> None:
        row = self.source_poll_state.setdefault(source_id, {})
        row["last_polled_at"] = now_utc().isoformat()
        row["last_status"] = "ok" if ok else "error"
        row["last_fetched"] = int(fetched)
    ...
    def batch_create_manual_messages(
        self,
    ...
    def signal_for_symbol(self, symbol: str) -> tuple[str, float, str]:
        ...
        signal, base_conf = aggregate_signal(
            long_score,
            short_score,
            buy_threshold=float(self.signal_thresholds.get("buy_net_threshold", 12.0)),
            sell_threshold=float(self.signal_thresholds.get("sell_net_threshold", -12.0)),
        )
        return signal, round((base_conf + confidence) / 2, 4), f"samples={samples}, sentiment={sentiment}"

    def update_signal_thresholds(self, payload: dict[str, Any], *, actor: str = "system") -> dict[str, Any]:
        buy = payload.get("buy_net_threshold")
        sell = payload.get("sell_net_threshold")
        if buy is not None:
            self.signal_thresholds["buy_net_threshold"] = float(buy)
        if sell is not None:
            self.signal_thresholds["sell_net_threshold"] = float(sell)
        self._audit("signal.thresholds.update", actor, deepcopy(self.signal_thresholds))
        self._persist_state()
        return deepcopy(self.signal_thresholds)
    ...
    def event_features(self, event_id: str) -> dict[str, Any] | None:
        event = self.events.get(event_id)
        if not event:
            return None
        source = self._source_by_id.get(event.source_id, {"source_id": event.source_id})
        confidence_avg = round(sum(item.confidence for item in event.impacts) / max(len(event.impacts), 1), 4)
        return {
            "event_id": event_id,
            "feature_vector": {
                "importance_score": event.importance_score,
                "importance_level_rank": self._alert_level_rank(event.importance_level),
                "market_span": len(set(event.impacted_markets)),
                "impacts_count": len(event.impacts),
                "avg_confidence": confidence_avg,
                "source_weight_effective": round(calculate_effective_source_weight(source), 4),
    ...
    def list_notifications(self, *, channel: str | None = None, status: str | None = None, limit: int = 100) -> list[dict[str, Any]]:
        rows = []
        for item in reversed(self.notification_outbox):
    ...
    def metrics_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "notifications_total": len(self.notification_outbox),
            "notifications_queued": sum(1 for item in self.notification_outbox if item.get("status") == "queued"),
            "notifications_delivered": sum(1 for item in self.notification_outbox if item.get("status") == "delivered"),
            "webhook": webhook,
        }
    ...
    def import_state_snapshot(self, payload: dict[str, Any], *, merge: bool = False) -> dict[str, Any]:
        ...
        self.source_poll_state.update(dict(payload.get("source_poll_state", {})))
